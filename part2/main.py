...
def handle_client(num, len_, udp_port, secretA, client_addr, student_id):
    ...
    while True:
        try:
            ...
            print(
                f"Did not receive any packets for {TIMEOUT} seconds. Closing socket on {udp_port}"
            )
            udp_socket.close()
            return
        except struct.error:
...
while True:
    try:
        ...
        if (
            data[HEADERSIZE : HEADERSIZE + payload_len] == b"hello world\0"
        ):  # Only for stage A
    ...
    if received_something:
        ...
        t = threading.Thread(
            ...
            args=(num, len_, udp_port, secretA, client_addr, student_id),
        )
        t.start()
